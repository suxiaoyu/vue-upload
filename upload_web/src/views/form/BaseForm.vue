    ...
    <div class="app-container">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>新建公文</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
  ...
  export default {
    ...
    //分析上级依赖文件
     firstToSecond() {
          if (!this.form.name || !this.form.desc) {
              this.$message.warning('公文名称、内容均不能为空!')
              return
          }
          //处理公文的上级审核列表
          const para = {
          ...
          parseDocx(para).then(res => {
            ...
            if(res.msg=='文档解析失败！')
            {
              this.$message.warning('文档解析失败!')
              return
            }
            //增加公文列表
            extractSupDoc(doc_id).then(res => {
                this.supDoc=[]           
                for(let key in res.sup_doc_list)
                {
                    
                     var index=key
                     var name=res.sup_doc_list[key][0]
                     var show_upload=false
                     var exists=getExistText(res.sup_doc_list[key][1],res.sup_doc_list[key][2])
                    
                     this.supDoc.push({'index':index,'name':name,is_correct:res.sup_doc_list[key][1],is_analysised:res.sup_doc_list[key][2],'exist':exists,'show_upload':show_upload})
                }
                
            })
            
             this.tabValueSecond='true';
             this.tabValue='second';
          })
         
        },
      //判断上级公文正确与否
     updateSupDocx(index,flag,row) {
         const para = {
            inf_doc_id: this.doc_id,
            sup_doc_title:row.name,
            is_correct:flag
          }
          //处理公文的上级审核列表
          var change_flag=0
          updateSupDoc(para).then(res => {
            if(flag==1)//正确的上级公文需要判断是否需要上传文档
            {
               this.supDoc[index]['is_correct']=1
                
            }
            else//错误文档，修改文档参数就好
            {
               this.supDoc[index]['is_correct']=0
            }
            var exists=getExistText(this.supDoc[index]['is_correct'],this.supDoc[index]['is_analysised'])
            this.supDoc[index]['exist']=exists
            this.$message({
                  message: '审核成功!',
                  type: 'success'
                 })
    ...
    //分析上级依赖文件
     secondToThird() {
          ...
          //处理增加新闻
          fetchMatchingResult(para).then(res => {
            this.matchingResult=[]         
            for(let key in res.matching_results)
            {
                this.matchingResult.push({
                'start':parseInt(key)+1,
                'matchingId':res.matching_results[key][0],
                'supName':res.matching_results[key][1],
                'original':res.matching_results[key][2],
                'target':res.matching_results[key][3],
                'score':res.matching_results[key][6],
                'is_correct':res.matching_results[key][7]})
                   /*  this.matchingResult[key]={}
                     this.matchingResult[key]['start']=parseInt(key)+1
                     this.matchingResult[key]['edit']=0
                     this.matchingResult[key]['matchingId']=res.matching_results[key][0]
                     this.matchingResult[key]['supName']=res.matching_results[key][1]
                     this.matchingResult[key]['original']=res.matching_results[key][2]
                     this.matchingResult[key]['target']=res.matching_results[key][3]
                     this.matchingResult[key]['score']=res.matching_results[key][6]*/
            }
            this.loading=false
           })
          this.tabValueThird='true';
          this.tabValue='third';
        },
         //判断审核结果正确与否
       updateResult(index,flag,rowIndex) {
         const para = {
            matching_id: index,
            flag:flag
          }
          //处理增加新闻
          userConfirmMatching(para).then(res => {
              //修改前端页面
              this.matchingResult[rowIndex]['is_correct']=flag
              this.$message({
                  message: '修改成功',
                  type: 'success'
                })
